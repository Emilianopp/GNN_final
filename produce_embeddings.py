...
class DyGFormer(nn.Module):
    ...
    def __init__(self, node_raw_features: np.ndarray, edge_raw_features: np.ndarray, neighbor_sampler: NeighborSampler,
        ...
        self.output_layer = nn.Linear(in_features=self.num_channels * self.channel_embedding_dim, out_features=3, bias=True)
    ...
    def compute_src_dst_node_temporal_embeddings(self, src_node_ids: np.ndarray, dst_node_ids: np.ndarray, node_interact_times: np.ndarray,before = True):
        ...
        if not isinstance(src_node_ids,np.int64) and not isinstance(dst_node_ids,np.int64) and  src_node_ids.shape[0] < node_interact_times.shape[0]:
        ...
        return self.sigmoid(src_node_embeddings), self.sigmoid(dst_node_embeddings)
...
edge_raw_features = full_datas[0]['edge_features']
node_raw_features = full_datas[0]['node_features']
...
train_neg_edge_sampler = NegativeEdgeSampler(src_node_ids=full_data_unstacked.src_node_ids, dst_node_ids=full_data_unstacked.dst_node_ids,negative_sample_strategy='random',interact_times=full_data_unstacked.node_interact_times,seed = 100)
...
Gs = defaultdict(dict)
d = pd.read_csv('/home/emiliano/projects/def-cbravo/emiliano/DyGLib/processed_data/Citation/labels.csv')
from IPython.display import display
...
for ts in snapshots :
    ...
    print('In ts loop The CPU usage is: ', psutil.cpu_percent(4))
    for batch_idx, full_data_indices in enumerate(full_idx_data_loader_tqdm):
        ...
        dst_ids = batch_dst_node_ids
 
        for k,(u,v) in enumerate(zip(ids,dst_ids)):
            embeddings[ts][u] = batch_src_node_embeddings[k].detach().cpu().numpy()
            embeddings[ts][v] = batch_dst_node_embeddings[k].detach().cpu().numpy()
...
with open(f'./embeddings/emb.pkl', 'wb+') as f:
