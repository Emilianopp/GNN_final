...
import warnings
from utils.DataLoader import get_link_prediction_data_snapshots, get_snpashot_idx_data_loaders
...
from utils.evaluate_models_utils import evaluate_model_link_prediction,evaluate_model_link_prediction_snapshot
...
print('got full sampler')




train_neg_edge_sampler = NegativeEdgeSampler(src_node_ids=train_data_unstacked.src_node_ids, dst_node_ids=train_data_unstacked.dst_node_ids,negative_sample_strategy='random',interact_times=train_data_unstacked.node_interact_times)
val_neg_edge_sampler = NegativeEdgeSampler(src_node_ids=full_data_unstacked.src_node_ids, dst_node_ids=full_data_unstacked.dst_node_ids,negative_sample_strategy='random',interact_times=full_data_unstacked.node_interact_times,seed = 10)
test_neg_edge_sampler = NegativeEdgeSampler(src_node_ids=full_data_unstacked.src_node_ids, dst_node_ids=full_data_unstacked.dst_node_ids,negative_sample_strategy='random',interact_times=full_data_unstacked.node_interact_times,seed = 100)

print('got negative samplers')
...
dynamic_backbone = TGAT(node_raw_features=None, edge_raw_features=None, neighbor_sampler=train_neighbor_sampler,
...
for epoch in range(args.num_epochs):
    ...
    for j,ts in enumerate(snapshots) :
        ...
        train_idx_data_loader = full_data_loaders[ts]
        # store train losses and metrics
        train_data = full_datas[ts]['edges']
        ...
        val_losses, val_metrics = evaluate_model_link_prediction_snapshot(model_name=args.model_name,
                                                            ...
                                                            evaluation_data_loaders= val_data_loaders,before  = args.before, ts = ts)


        logger.info(f'Epoch: {epoch + 1}, batch {batch_idx},  learning rate: {optimizer.param_groups[0]["lr"]}, train loss: {np.mean(train_losses):.4f}')
    ...
    for j,ts in enumerate(snapshots) :
        test_losses, test_metrics = evaluate_model_link_prediction_snapshot(model_name=args.model_name,
                                                        ...
                                                        num_neighbors=args.num_neighbors,
                                                        time_gap=args.time_gap,
                                                        evaluation_data_loaders= test_data_loaders,before = args.before,ts =ts)
