...
def evaluate_model_link_prediction_snapshot(model_name: str, model: nn.Module, neighbor_sampler: NeighborSampler, evaluation_data_loaders,
                                   ...
                                   num_neighbors: int = 20, time_gap: int = 2000, before = True,ts = None):
    """
    evaluate models on the link prediction task
    :param model_name: str, name of the model
    :param model: nn.Module, the model to be evaluated
    ...
    with torch.no_grad():
        ...
        for batch_idx, evaluate_data_indices in enumerate(evaluate_idx_data_loader_tqdm):
            ...
            if evaluate_neg_edge_sampler.negative_sample_strategy != 'random':
                ...
                if isinstance( batch_src_node_ids,np.int64):
                    size = 1
                    continue
                else:
                    size = len(batch_src_node_ids)
                _, batch_neg_dst_node_ids = evaluate_neg_edge_sampler.sample(size=size)
                batch_neg_src_node_ids = batch_src_node_ids
            ...
            labels = torch.cat([torch.ones(batch_src_node_embeddings.shape[0]), torch.zeros(batch_neg_src_node_embeddings.shape[0])], dim=0).to(torch.device('cuda'))

            batch_src_node_embeddings = torch.cat((batch_src_node_embeddings,batch_neg_src_node_embeddings),dim = 0 )
            batch_dst_node_embeddings = torch.cat((batch_dst_node_embeddings,batch_neg_dst_node_embeddings),dim= 0)

            predicts = torch.sum(batch_dst_node_embeddings * batch_dst_node_embeddings, dim = 1)
