...
def create_optimizer(model: nn.Module, optimizer_name: str, learning_rate: float, weight_decay: float = 0):
    """
    create optimizer
    :param model: nn.Module
...
class NeighborSampler:
    ...
    def find_neighbors_current(self, node_id: int, interact_time: float,num_neighbors = 5, return_sampled_probabilities: bool = False,dst_node_ids = None ):
        """
        extracts all the interactions happening before interact_time (less than interact_time) for node_id in the overall interaction graph
        the returned interactions are sorted by time.
        :param node_id: int, node id
        ...
        neighbor_ids = self.nodes_neighbor_ids[node_id][:i][:num_neighbors]
       
  
    
        edges_ids = self.nodes_edge_ids[node_id][:i][:num_neighbors]
        nodes_neighbor_times =self.nodes_neighbor_times[node_id][:i][:num_neighbors]
    ...
    def get_historical_neighbors(self, node_ids: np.ndarray, node_interact_times: np.ndarray, num_neighbors: int = 20):
        ...
        for idx, (node_id, node_interact_time) in enumerate(zip(node_ids, node_interact_times)):
            ...
            node_neighbor_ids, node_edge_ids, node_neighbor_times, node_neighbor_sampled_probabilities = \
                self.find_neighbors_current(node_id=node_id, interact_time=node_interact_time)
    ...
    def get_all_first_hop_neighbors(self, node_ids: np.ndarray, node_interact_times: np.ndarray,num_neighbors = 5,dst_node_ids  = None):
        """
        ...
        :param node_ids: ndarray, shape (batch_size, ), node ids
        :param node_interact_time
        s: ndarray, shape (batch_size, ), node interaction times
        :return:
        """
