...
class TGAT(nn.Module):
    ...
    def __init__(self, node_raw_features: np.ndarray, edge_raw_features: np.ndarray, neighbor_sampler: NeighborSampler,
        ...
        self.reduction_layer1 = nn.Linear(self.node_feat_dim,int((self.node_feat_dim)/2))
        self.reduction_layer2 = nn.Linear(int((self.node_feat_dim)/2),3,bias = False)
        self.act = nn.ReLU()
        self.sof = nn.ReLU()

    def compute_src_dst_node_temporal_embeddings(self, src_node_ids: np.ndarray, dst_node_ids: np.ndarray,
                                                 node_interact_times: np.ndarray, num_neighbors: int = 5):
        """
        ...
        src_node_embeddings = self.act(self.reduction_layer2  (self.act(self.reduction_layer1(src_node_embeddings))))
        # Tensor, shape (batch_size, node_feat_dim)
        dst_node_embeddings = self.compute_node_temporal_embeddings(node_ids=dst_node_ids, node_interact_times=node_interact_times,
                                                                    current_layer_num=self.num_layers, num_neighbors=num_neighbors)
        dst_node_embeddings = self.act(self.reduction_layer2  (self.act(self.reduction_layer1(dst_node_embeddings))))
